...
cd rv
go build -o loli .
# Windows の場合: go build -o loli.exe .
```

`loli` を PATH の通った場所に配置。まず Lolit アカウントにログインします（WebUIで作成したユーザー名/パスワード）。
...
loli push
loli lock arm_link1.SLDPRT
```

セットアップ後の動作確認には `loli doctor` が便利です。git / git-lfs のインストール状況、
...
`LOLIT_SERVER` 環境変数でLolitサーバーのURLを指定する（未設定時は `http://localhost:8080`）。

SolidWorks 起動後、Lolit ツールバーから：

1. 「Lolitにログイン」でLolitアカウントにログイン（初回のみ）
2. ファイルを編集したら「Lolitに保存」— リポジトリ全体を1バージョンとして保存・共有する
3. 「バージョン履歴」— 過去バージョンの一覧表示、単体または関連部品込みでのダウンロード

Git用語は一切出てこない。アセンブリの部品参照は保存時に自動で記録されるため、
過去バージョンをダウンロードしても参照が壊れない。
