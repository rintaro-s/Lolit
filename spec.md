...
> **重要: Lolit は Git ではない。** Lolit はロボコン開発チーム向けの独自のファイル管理システムであり、
> バックエンドをゼロから作るのが手間なため実装の都合上 Git + Gitea を流用しているに過ぎない。
> ユーザー（WebUI利用者、SolidWorks/KiCADプラグイン利用者、Lolit CLI利用者）から見える概念は
> 「バージョン」「保存」「ダウンロード」であり、「コミット」「ブランチ」「プッシュ」といったGit用語は
> 実装の内部詳細としてのみ存在する。UIやドキュメントで新しく文言を追加するときはこれを厳守すること。

## 1. 概要

Lolitは、ロボコン開発チーム向けのファイル管理システムである。バージョン管理・保存・共有の実体には
Git（+ Gitea）を採用しているが、これは実装上の選択に過ぎず、ユーザーに対しては独自のファイル管理システム
として振る舞う。CAD（SolidWorks / KiCAD）に特化した差分表示・検索・ロック機能を追加する。
...
### 4.3.1 SolidWorks Addin: バージョン履歴・依存関係・ダウンロード（確定）

SolidWorks は完結したGUIクライアントとして動作し、Gitを一切意識させない。ツールバーから以下を提供する。

| コマンド | 内容 |
|---|---|
| Lolitにログイン | Lolitアカウントでログイン（トークンをOSのアプリケーションデータフォルダに保存） |
| Lolitに保存 | 現在開いているリポジトリ全体の変更を1つのバージョンとして保存し共有する（内部的には該当フォルダ全体を`git add -A && git commit && git push`する。ユーザーには「バージョンID」のみ見せる） |
| バージョン履歴 | 現在のファイルの保存履歴を一覧表示し、任意の過去バージョンをダウンロードする |

**アセンブリの依存関係（キモの部分）**

SolidWorksのアセンブリファイルは他の部品/サブアセンブリファイルへの参照を持ち、依存関係を無視して
古いバージョンのアセンブリだけを取り出しても正しく開けない。これを解決するため：

1. 「Lolitに保存」実行時、Addinはアセンブリツリー全体（`Component.Children`を再帰的にたどる）を走査し、
   各アセンブリ／サブアセンブリについて「直接参照している部品・サブアセンブリのリポジトリ内相対パス」を
   `POST /api/dependencies`でサーバへ送信する。すべて同一の保存操作でコミットされるため、
   参照先の版も同じバージョンIDで確定する（≒ロックファイル方式）。
2. ダウンロード時（`GET /api/resolve` / `GET /api/download-bundle`）、サーバは記録済みの依存関係グラフを
   再帰的にたどり、「そのアセンブリを開くのに必要な全ファイル、各ファイルごとの正確なバージョン」を解決する。
   サイクル（循環参照）が発生しても無限ループしないよう既訪問集合で防御する。
3. 「関連部品も含めてダウンロード」を選ぶと、解決された全ファイルを相対パス構造を保ったままZIPで受け取り、
   選択フォルダに展開する。これによりSolidWorksで正しく開ける状態が再現される。
4. 単一ファイルのダウンロード（`GET /api/download`）は、Git LFSポインタを自動的に実体（Giteaの
   LFSオブジェクトストア）へ解決してから返す。ユーザーやAddinがLFSポインタを直接見ることはない。

トップレベルのアセンブリで「Lolitに保存」を1回実行するだけで、SolidWorksが読み込んでいる
コンポーネントツリー全体（サブアセンブリ・孫アセンブリを含む）を辿って依存関係を記録するため、
別途サブアセンブリを個別に開いて保存し直す必要はない。ただし軽量化（Lightweight）表示等で
コンポーネントツリーが完全に読み込まれていない場合、その部分の依存関係は記録されない点に注意。
...
### 5.6 dependencies テーブル（依存関係グラフ、確定）

| カラム | 型 | 内容 |
|---|---|---|
| id | INTEGER PK | |
| repo | TEXT | |
| path | TEXT | 依存元ファイルのリポジトリ内相対パス |
| version | TEXT | 依存元ファイルのバージョン |
| dep_path | TEXT | 依存先ファイルのリポジトリ内相対パス |
| dep_version | TEXT | 依存先ファイルの、依存元保存時点でのバージョン |

`(repo, path, version)`の組で直接依存エッジを保持する。同じ組への再保存は既存エッジを全置換する
（保存し直した時点の依存関係が常に正となるため）。
...
### 9.1 バージョン単位のダウンロードAPI（確定）

WebUIとSolidWorks Addinの両方から使われる、共通のダウンロード系API。

| エンドポイント | 内容 |
|---|---|
| `POST /api/dependencies` | `{repo, path, version, deps: [{path, version}]}` — あるファイルが特定バージョン時点で依存する他ファイルの版を記録する（4.3.1節参照） |
| `GET /api/resolve?repo=&path=&version=` | 依存関係グラフを再帰的に解決し、必要なファイルと各バージョンの一覧を返す |
| `GET /api/download?repo=&path=&version=` | 単一ファイルの中身をそのバージョン時点の内容で返す。Git LFSポインタは自動的に実体へ解決される |
| `GET /api/download-bundle?repo=&path=&version=` | `resolve`の結果をもとに、依存ファイル一式を相対パス構造を保ったままZIPで返す |

これらのエンドポイントの外部インターフェース（クエリパラメータ名・レスポンスのJSONキー）は
意図的に `version` という言葉を使い、内部実装が使う `commit`（gitのコミットハッシュ）という言葉を
表に出さない。
