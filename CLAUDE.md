...
- `rv/` — Go CLI (`loli`), a thin wrapper around `git`/`git-lfs` plus Lolit-specific commands
...
make cli      # cd rv && go build -o loli .
...
# loli CLI
cd rv && go build -o loli . && ./loli version
cd rv && go test ./...
...
```
Client
 ├── loli CLI (Go)              — git/git-lfs wrapper + lock/search/history/release commands
...
- **No native GUI client.** The WebUI is the single cross-platform GUI; `loli` is the CLI; SolidWorks/
